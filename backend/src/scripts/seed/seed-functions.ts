...
export async function createStore(
  ...
  // Update store with region, sales channel, and currency
  ...
  console.log('Store currency set to PHP')
}
export async function createRegions(container: MedusaContainer) {
  const regionModuleService = container.resolve(Modules.REGION)
  
  // Check if Philippines region already exists
  ...
  if (existingRegions.length > 0) {
    // Region exists, use existing region
    region = existingRegions[0]
    console.log(`Philippines region already exists with ID: ${region.id}. Currency is: ${region.currency_code}`)
    
    // If currency is not PHP, we would need to update it through admin or database directly
    if (region.currency_code !== 'php') {
      console.log(`Warning: Region currency is ${region.currency_code}, not PHP. Please update through admin panel.`)
    }
    
    console.log('Note: Run the update-region-payment-providers script after backend starts to add GiyaPay')
  } else {
    // Create new region - GiyaPay will be added later via update script
    const {
      result: [regionResult]
  ...
  // Handle tax regions - only create if they don't exist
  try {
    ...
    console.log('Tax regions already exist, skipping creation.')
  }
    ...
    input: [
      {
        ...
        type: {
          ...
          code: sellerName,
          description: 'Philippines shipping'
        },
        rules: [
