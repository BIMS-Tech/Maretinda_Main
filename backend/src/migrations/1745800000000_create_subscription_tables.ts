...
export class Migration1745800000000 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(`
      CREATE TABLE IF NOT EXISTS "seller_subscription" (
        ...
        "created_at"        timestamptz NOT NULL DEFAULT now(),
        "updated_at"        timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "seller_subscription_pkey" PRIMARY KEY ("id")
      );
    `)

    this.addSql(`CREATE INDEX IF NOT EXISTS "seller_subscription_seller_id_idx" ON "seller_subscription" ("seller_id");`)
    this.addSql(`CREATE INDEX IF NOT EXISTS "seller_subscription_status_idx"    ON "seller_subscription" ("status");`)
    this.addSql(`CREATE INDEX IF NOT EXISTS "seller_subscription_end_date_idx"  ON "seller_subscription" ("end_date");`)
  ...
  async down(): Promise<void> {
    this.addSql(`DROP TABLE IF EXISTS "seller_subscription";`)
