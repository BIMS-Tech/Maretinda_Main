...
const UserBadge = () => {
  ...
  if (isError) {
    // Handle session error gracefully instead of throwing
    console.warn("User session error:", error)
    return (
      <button
        onClick={() => window.location.href = '/login'}
        className="shadow-borders-base flex max-w-[192px] select-none items-center gap-x-2 overflow-hidden text-ellipsis whitespace-nowrap rounded-full py-1 pl-1 pr-2.5 bg-red-50 hover:bg-red-100"
      >
        <div className="h-5 w-5 rounded-full bg-red-200 flex items-center justify-center text-xs text-red-600">
          !
        </div>
    ...
    <div className="p-3">
      ...
      >
        <div className="flex size-7 items-center justify-center">
          {fallback ? (
            <ImageAvatar src={avatar} fallback={displayName} size={7} rounded />
          ) : (
...
const UserItem = () => {
  ...
  if (isPending) {
    return (
      <div className="p-3">
        <div className="shadow-borders-base flex max-w-[192px] select-none items-center gap-x-2 overflow-hidden text-ellipsis whitespace-nowrap rounded-full py-1 pl-1 pr-2.5">
          <Skeleton className="h-5 w-5 rounded-full" />
          <Skeleton className="h-[9px] w-[70px]" />
        </div>
      </div>
  ...
  if (isError) {
    ...
    console.warn("User session error in UserItem:", error)
    return (
      <div className="p-3">
        <div className="shadow-borders-base flex max-w-[192px] select-none items-center gap-x-2 overflow-hidden text-ellipsis whitespace-nowrap rounded-full py-1 pl-1 pr-2.5 bg-red-50">
          <div className="h-5 w-5 rounded-full bg-red-200 flex items-center justify-center text-xs text-red-600">
            !
          </div>
          <span className="text-xs text-red-600">Session Error</span>
  ...
  return (
    <div className="flex items-center gap-x-3 overflow-hidden px-2 py-1">
      <div className="w-10">
        <ImageAvatar src={avatar} fallback={displayName} size={8} rounded />
      </div>
