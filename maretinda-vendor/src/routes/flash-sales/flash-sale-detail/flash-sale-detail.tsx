        ...
        <div className="absolute z-50 top-full mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-64 overflow-y-auto">
          ...
          {filtered.map((product: any) => (
            <button
              ...
              onClick={() => {
                onSelect({ id: product.id, title: product.title, thumbnail: product.thumbnail })
                setQuery("")
                setOpen(false)
              }}
            >
...
function AddItemForm({ flashSaleId, existingIds, onAdded }: {
  flashSaleId: string
  existingIds: string[]
  onAdded: () => void
}) {
  ...
  const handleAdd = () => {
    ...
    if (!discountValue || parseFloat(discountValue) <= 0) { toast.error("Enter a valid discount value"); return }
  ...
  return (
    <div className="border border-dashed border-violet-200 rounded-xl p-5 bg-violet-50/40 space-y-4">
      <p className="text-sm font-semibold text-gray-700">Add Product to Flash Sale</p>
      ...
      {/* Step 2 — discount settings (shown only after product selected) */}
      {selected && (
      ...
      {selected && (
        <Button size="small" onClick={handleAdd} isLoading={isPending}>
          <Plus className="w-3.5 h-3.5 mr-1" /> Add to Flash Sale
        </Button>
      )}
    </div>
  )
        ...
        {editable && !editing && (
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              ...
              onClick={async () => {
                const ok = await prompt({ title: "Remove", description: "Remove this product from the flash sale?", confirmText: "Remove", cancelText: "Cancel" })
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {editable && (
            <Button size="small" variant="secondary" onClick={() => setShowAddItem((v) => !v)}>
              <Plus className="w-3.5 h-3.5 mr-1" /> {showAddItem ? "Cancel" : "Add Product"}
            </Button>
          )}
        </div>
