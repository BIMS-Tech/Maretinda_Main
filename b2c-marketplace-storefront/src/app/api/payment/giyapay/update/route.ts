...
export async function POST(req: NextRequest) {
  try {
    ...
    const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || process.env.BACKEND_URL || 'http://localhost:9000'
    ...
    const resp = await fetch(`${backendUrl}/store/giyapay/update`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(authHeaders || {}), ...(publishable ? { 'x-publishable-api-key': publishable } : {}) },
      body: JSON.stringify(body)
    })
    const data = await resp.json().catch(() => ({}))
    return NextResponse.json(data, { status: resp.status })
  } catch (e: any) {
    return NextResponse.json({ error: 'forward_failed', message: e.message }, { status: 500 })
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    let payload: any = {
      reference_number: query.reference_number || query.refno || query.ref || query.referenceNumber,
      order_id: query.order_id || query.orderId,
      cart_id: query.cart_id || query.cartId,
      gateway: (query.gateway || query.channel || query.provider || query.payment_method || '').toString().toUpperCase(),
    ...
    if ((!payload.vendor_id || !payload.vendor_name) && payload.order_id) {
      try {
        ...
        if (orderRes.ok) {
          ...
          payload.vendor_id = payload.vendor_id || seller?.member_id || seller?.id || seller?.seller_id
          payload.vendor_name = payload.vendor_name || seller?.name || seller?.store_name || seller?.store?.name
        }
      } catch {}
    }
    ...
    const authHeaders = await getAuthHeaders()
    const publishable = process.env.NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY || process.env.NEXT_PUBLIC_MEDUSA_PUBLISHABLE_API_KEY
    const resp = await fetch(`${backendUrl}/store/giyapay/update`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(authHeaders || {}), ...(publishable ? { 'x-publishable-api-key': publishable } : {}) }, body: JSON.stringify(payload) })
    const data = await resp.json().catch(() => ({}))
    return NextResponse.json(data, { status: resp.status })
  } catch (e: any) {
    return NextResponse.json({ error: 'forward_failed', message: e.message }, { status: 500 })
