  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const res = await fetch(`${backendUrl}/store/giyapay/transaction?order_id=${order.id}`, {
          headers: {
            'accept': 'application/json',
            ...(publishable ? { 'x-publishable-api-key': publishable } : {}),
          }
        })
        if (res.ok) {
        ...
        const orderRes = await fetch(`${backendUrl}/store/orders/${order.id}?fields=%2Apayment_collections.payments`, {
          headers: {
            'accept': 'application/json',
            ...(publishable ? { 'x-publishable-api-key': publishable } : {}),
          }, cache: 'no-store'
        ...
        if (orderRes.ok) {
          ...
          const providers: string[] = (o?.payment_collections?.payments || []).map((p: any) => p?.provider_id).filter(Boolean)
        ...
        >
          ...
          {isCod && (
            <div className="mt-2 border rounded-md p-4 bg-ui-button-neutral">
              <Text className="font-medium">Payment on Delivery</Text>
              <Text className="text-ui-fg-subtle">Please prepare cash upon delivery. No payment was captured online.</Text>
            </div>
          )}
          {txn && (
            <div className="mt-2 grid grid-cols-2 gap-2 border rounded-md p-4">
              <Text className="font-medium">Reference</Text>
              <Text className="font-mono truncate">{txn.referenceNumber}</Text>
              <Text className="font-medium">Amount</Text>
              <Text>{new Intl.NumberFormat('en-PH', { style: 'currency', currency: txn.currency || 'PHP' }).format(txn.amount || 0)}</Text>
