  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => {
          const isCod = Array.isArray(row.original?.payment_collections?.payments)
            ? row.original.payment_collections.payments.some((p: any) => p?.provider_id === 'pp_system_default')
            : false
          const status = isCod ? 'awaiting' : (row.original?.payment_status || '-')
