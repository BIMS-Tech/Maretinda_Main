...
export const GET = async (
  ...
  try {
    ...
    console.log('[Payout Account] Error fetching account, using mock data:', error.message)
    
    // Return mock payout account if seller has DFT information
    const hasDftInfo = seller.dft_bank_name && seller.dft_account_number && seller.dft_swift_code
    
    const mockPayoutAccount = {
      id: `pa_${seller.id}`,
      seller_id: seller.id,
      status: hasDftInfo ? 'active' : 'inactive',
      account_type: 'express',
      currency: 'PHP',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
      metadata: {
        bank_name: seller.dft_bank_name,
        account_number: seller.dft_account_number,
    ...
    res.json({
      payout_account: mockPayoutAccount
    })
  }
}
