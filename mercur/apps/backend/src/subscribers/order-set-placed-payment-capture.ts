...
export default async function orderSetPlacedHandler({
  ...
  // Skip auto-capturing for COD/manual payments. Vendors will capture manually later.
  const hasManualCodPayment = Array.isArray(payment_collection.payments)
    ? payment_collection.payments.some((p: any) => p?.provider_id === 'pp_system_default')
    : false

  if (hasManualCodPayment) {
    // Leave payment status as awaiting until vendor marks as captured
    return
  }
